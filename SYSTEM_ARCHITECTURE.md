# Pixel Controller System Architecture
...
- This architecture file is for system architecture, implementation status,
  hardware mapping, RAM/protocol decisions, progress, and known issues.
- The practical Chinese handoff and operation manual is maintained in
  `CODEX_HANDOFF.md`.
...
- Startup flow:
  ...
  - if any connected board reports nonzero `rc_event_bits`, stop waiting for
    missing boards and force playback immediately.
- RC mapping:
  - bit0 -> `mode1.pixelbin`,
  - bit1 -> `mode2.pixelbin`,
  - bit2 -> `black.pixelbin`,
  - bit3 -> pause, which stops submitting RGB frames and keeps current output.
- During playback, RC events are polled periodically and action switches take
  effect at frame boundaries. Repeating the currently active action is ignored.
...
- If playback is forced by RC before all boards are present, the first version
  ...
  mapping.
- Firmware protocol now reports both `rc_stable_bits` and `rc_event_bits` in
  `STATUS_RSP`. `rc_event_bits` contains debounced press events and is cleared
  after STATUS reports it.
